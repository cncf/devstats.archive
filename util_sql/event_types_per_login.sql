select
  ...
  case date_part('year', sub.dt_from) when 1900 then '-' else to_char(sub.dt_from, 'MM/DD/YYYY') end as date_from,
  case date_part('year', sub.dt_to) when 2100 then '-' else to_char(sub.dt_to, 'MM/DD/YYYY') end as date_to,
