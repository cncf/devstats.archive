...
GHA2DB_PROJECT=screwdrivercd PG_DB=screwdrivercd ./shared/import_affs.sh 2>>errors.txt | tee -a run.log || exit 6
GHA2DB_PROJECT=screwdrivercd PG_DB=screwdrivercd ./shared/setup_scripts.sh 2>>errors.txt | tee -a run.log || exit 7
GHA2DB_PROJECT=screwdrivercd PG_DB=screwdrivercd ./shared/get_repos.sh 2>>errors.txt | tee -a run.log || exit 8
GHA2DB_PROJECT=screwdrivercd PG_DB=screwdrivercd GHA2DB_LOCAL=1 vars || exit 9
./devel/ro_user_grants.sh screwdrivercd || exit 10
./devel/psql_user_grants.sh devstats_team screwdrivercd || exit 11
