...
sudo -u postgres psql harbor -c "create extension if not exists pgcrypto" || exit 1
GHA2DB_PROJECT=harbor PG_DB=harbor GHA2DB_LOCAL=1 ./gha2db 2016-03-02 0 today now vmware harbor 2>>errors.txt | tee -a run.log || exit 2
