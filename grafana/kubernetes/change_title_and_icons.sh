...
then
  ...
  do
    ls -l "$f"
    vim -c --not-a-term "%s/'Grafana - '/'K8s DevStats - '/g|wq" "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '"Grafana - "' "{}" \; | sort | uniq`
  do
    ls -l "$f"
...
cp grafana/img/k8s.svg ${GRAFANA_DATA}/public/img/grafana_com_auth_icon.svg || exit 1
cp -n ${GRAFANA_DATA}/public/img/grafana_net_logo.svg ${GRAFANA_DATA}/public/img/grafana_net_logo.svg.bak
cp grafana/img/k8s.svg ${GRAFANA_DATA}/public/img/grafana_net_logo.svg || exit 1
