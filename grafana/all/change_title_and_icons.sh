#!/bin/bash
# GRAFANA_DATA=/usr/share/grafana.all/
if [ -z "${GRAFANA_DATA}" ]
then
...
then
  ...
  do
    ls -l "$f"
    vim --not-a-term -c "%s/'Grafana - '/'All DevStats - '/g" -c 'wq!' "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '"Grafana - "' "{}" \; | sort | uniq`
  do
    ls -l "$f"
  ...
  do
    ls -l "$f"
    vim --not-a-term -c "%s/' - Grafana'/' - All CNCF DevStats'/g" -c 'wq!' "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '" - Grafana"' "{}" \; | sort | uniq`
  do
    ls -l "$f"
