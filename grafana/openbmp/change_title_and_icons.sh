#!/bin/bash
# GRAFANA_DATA=/usr/share/grafana.openbmp/
...
then
  ...
  do
    ls -l "$f"
    vim -c "%s/'Grafana - '/'OpenBMP DevStats - '/g|wq" "$f"
  done
  for f in `find ${GRAFANA_DATA} -type f -exec grep -l '"Grafana - "' "{}" \; | sort | uniq`
  do
    ls -l "$f"
