...
5) You need to either regenerate all InfluxDB data (it takes about 10-15 minutes) using `PG_PASS=... IDB_PASS=... ./reinit_all.sh` or use `PG_PASS=... IDB_PASS=... ./add_single_metric,sh`. If You choose to use add single metric, You need to create 3 files: `gaps.yaml` (if empty copy from metrics/empty.yaml), `annotations.yaml` (usually empty) and `metrics.yaml`. Those YAML files should contain only new metric related data.
