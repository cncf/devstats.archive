...
docker run --mount src="/data/devstats",target="/root",type=bind -e INIT=1 -e SKIPTEMP=1 -e NOLOCK=1 -e NOBACKUP=1 -e SKIPADDALL=1 -e GHA2DB_GITHUB_OAUTH="${GHA2DB_GITHUB_OAUTH}" -e GHA2DB_GHAPISKIP="${GHA2DB_GHAPISKIP}" -e PG_PORT=65432 -e TEST_SERVER=1 -e PG_HOST="${host}" -e PG_PASS="${PG_PASS}" -e PG_PASS_RO="${PG_PASS_RO}" -e PG_PASS_TEAM="${PG_PASS_TEAM}" -e GHA2DB_DEBUG="${GHA2DB_DEBUG}" -e GHA2DB_CMDDEBUG="${GHA2DB_CMDDEBUG}" -e GHA2DB_QOUT="${GHA2DB_QOUT}" -it devstats ./docker/docker_deploy_all.sh
