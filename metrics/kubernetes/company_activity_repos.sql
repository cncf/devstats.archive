select
  concat('company;', sub.company, '`', sub.repo, ';activity,authors,issues,prs,pushers,pushes,reviews,comments,contributions,contributors'),
...
from (
  ...
  from
    ...
    gha_events ev
  where
    ev.dup_repo_name in (select repo_name from trepos)
