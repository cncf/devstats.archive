...
./devel/db.sh psql spire -c "create extension if not exists pgcrypto" || exit 1
GHA2DB_PROJECT=spire PG_DB=spire GHA2DB_LOCAL=1 gha2db 2017-09-28 0 today now 'regexp:(?i)^(spiffe\/spire.*)$' 2>>errors.txt | tee -a run.log || exit 2
