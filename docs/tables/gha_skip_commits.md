...
- To see code that excludes commit files, search for `filesSkipPattern` [here](https://github.com/cncf/devstats/blob/master/cmd/get_repos/get_repos.go). If commit has no files after filtering excluded ones, its SHA is added to `gha_skip_commits` table.
