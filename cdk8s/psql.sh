...
GHA2DB_PROJECT=cdk8s PG_DB=cdk8s ./shared/setup_scripts.sh 2>>errors.txt | tee -a run.log || exit 6
GHA2DB_PROJECT=cdk8s PG_DB=cdk8s ./shared/get_repos.sh 2>>errors.txt | tee -a run.log || exit 7
GHA2DB_PROJECT=cdk8s PG_DB=cdk8s GHA2DB_LOCAL=1 vars || exit 8
./devel/ro_user_grants.sh cdk8s || exit 9
./devel/psql_user_grants.sh devstats_team cdk8s || exit 10
