...
GHA2DB_PROJECT=crossplane PG_DB=crossplane ./shared/setup_scripts.sh 2>>errors.txt | tee -a run.log || exit 6
GHA2DB_PROJECT=crossplane PG_DB=crossplane ./shared/get_repos.sh 2>>errors.txt | tee -a run.log || exit 7
GHA2DB_PROJECT=crossplane PG_DB=crossplane GHA2DB_LOCAL=1 vars || exit 8
./devel/ro_user_grants.sh crossplane || exit 9
./devel/psql_user_grants.sh devstats_team crossplane || exit 10
