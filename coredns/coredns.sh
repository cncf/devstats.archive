...
GHA2DB_PROJECT=coredns IDB_DB=coredns PG_DB=coredns GHA2DB_LOCAL=1 ./gha2db 2016-03-18 0 2017-11-08 0 'miekg/coredns,coredns' 2>>errors.txt | tee -a run.log || exit 2
GHA2DB_PROJECT=coredns IDB_DB=coredns PG_DB=coredns GHA2DB_LOCAL=1 ./gha2db 2017-11-08 2 today now 'coredns' 2>>errors.txt | tee -a run.log || exit 3
...
./coredns/setup_repo_groups.sh 2>>errors.txt | tee -a run.log || exit 5
./coredns/import_affs.sh 2>>errors.txt | tee -a run.log || exit 6
./coredns/setup_scripts.sh 2>>errors.txt | tee -a run.log || exit 7
./coredns/get_repos.sh 2>>errors.txt | tee -a run.log || exit 8
