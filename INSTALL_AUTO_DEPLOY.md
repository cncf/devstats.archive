...
10. Install InfluxDB time-series database ([link](https://docs.influxdata.com/influxdb/v0.9/introduction/installation/)):
    ...
    - InfluxDB has authentication disabled by default: edit config file `vim /etc/influxdb/influxdb.conf` and change:
    - section `[http]`: `auth-enabled = true`, `max-body-size = 0`, `[subscriber]`: `http-timeout = "300s"`, `write-concurrency = 96`, `[coordinator]`: `write-timeout = "60s"`.
...
13. Install Apache & SSL (You need to have a working DNS name for this). This is optional.
...
14. Automatic deploy of first database (few examples)
    ...
    - To deploy other project using Influx backup from production specify `IGET=1` (get influx data form backup on `cncftest.io`). Specify GET=1 to get Postgres backup from `cncftest.io` too.
...
16. We need to setup cron job that will call sync every hour (10 minutes after 1:00, 2:00, ...)
