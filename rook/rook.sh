#!/bin/bash
# Run this script from the repository top level: ./rook/rook.sh
set -o pipefail
> errors.txt
> run.log
#GHA2DB_PROJECT=rook IDB_DB=rook PG_DB=rook GHA2DB_LOCAL=1 ./structure 2>>errors.txt | tee -a run.log || exit 1
#GHA2DB_EXCLUDE_REPOS='theupdateframework/notary' GHA2DB_PROJECT=rook IDB_DB=rook PG_DB=rook GHA2DB_LOCAL=1 ./gha2db 2016-11-07 0 today now 'rook' 2>>errors.txt | tee -a run.log || exit 2
