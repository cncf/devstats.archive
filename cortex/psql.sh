...
sudo -u postgres psql -h "$PG_HOST" -p "$PG_PORT" cortex -c "create extension if not exists pgcrypto" || exit 1
GHA2DB_PROJECT=cortex PG_DB=cortex GHA2DB_LOCAL=1 ./gha2db 2016-09-09 0 today now weaveworks 'cortexproject,weaveworks/cortex,weaveworks/prism,weaveworks/frankenstein' 2>>errors.txt | tee -a run.log || exit 2
