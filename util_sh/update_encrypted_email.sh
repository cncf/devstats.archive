...
GHA2DB_PROJECT=helm PG_DB=helm GHA2DB_LOCAL=1 ./update_encrypted_email 2015-10-06 0 today now 'helm,kubernetes-helm,kubernetes-charts,kubernetes/helm,kubernetes/charts,kubernetes/deployment-manager,kubernetes/application-dm-templates' 2>>errors.txt | tee -a run.log || exit 2
