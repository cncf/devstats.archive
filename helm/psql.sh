...
sudo -u postgres psql helm -c "create extension if not exists pgcrypto" || exit 1
GHA2DB_PROJECT=helm PG_DB=helm GHA2DB_LOCAL=1 ./gha2db 2015-10-06 0 today now 'helm,kubernetes-helm,kubernetes-charts,kubernetes/helm,kubernetes/charts,kubernetes/deployment-manager' 2>>errors.txt | tee -a run.log || exit 2
