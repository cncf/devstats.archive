...
> errors.txt
> run.log
GHA2DB_PROJECT=jaeger PG_DB=jaeger GHA2DB_LOCAL=1 ./structure 2>>errors.txt | tee -a run.log || exit 1
sudo -u postgres psql -h "$PG_HOST" -p "$PG_PORT" jaeger -c "create extension if not exists pgcrypto" || exit 1
...
GHA2DB_PROJECT=jaeger PG_DB=jaeger GHA2DB_LOCAL=1 GHA2DB_MGETC=y GHA2DB_SKIPTABLE=1 GHA2DB_INDEX=1 ./structure 2>>errors.txt | tee -a run.log || exit 3
