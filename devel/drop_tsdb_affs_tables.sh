...
sudo -u postgres psql $proj -c "drop table snum_stats" || exit 3
sudo -u postgres psql $proj -c "drop table scompany_activity" || exit 4
...
sudo -u postgres psql $proj -c "drop table ssex" || exit 8
sudo -u postgres psql $proj -c "drop table ssexcum" || exit 9
