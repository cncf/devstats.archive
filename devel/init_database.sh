...
exists=`sudo -u postgres psql -tAc "select 1 from pg_database WHERE datname = 'devstats'"` || exit 1
if ( [ ! -z "$LDROP" ] && [ "$exists" = "1" ] )
then
  echo "dropping postgres database devstats (logs)"
  sudo -u postgres psql -c "select pg_terminate_backend(pid) from pg_stat_activity where datname = 'devstats'" || exit 2
  sudo -u postgres psql -c "drop database devstats" || exit 3
fi
...
then
  ...
  sudo -u postgres psql -c "create database devstats" || exit 5
  sudo -u postgres psql -c "create user gha_admin with password '$PG_PASS'" || exit 6
