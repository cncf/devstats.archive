...
- Some projects should not be added to 'All CNCF' (like openconatiners), updated `devel/deploy_proj.sh` in such cases.
...
- Update `partials/projects.html`, `partials/projects_health.html`, `metrics/all/*vars.yaml`. Test with: `ONLY="proj1 proj2 ..." PG_PASS=... ./devel/vars_all.sh`. In simpler cases you can use `./util_sh/generate_partials.sh`.
- Update Apache proxy and SSL files `apache/www/index_* apache/*/sites-enabled/* apache/*/sites.txt` files.
- Run deploy all script: `SKIPTEMP=1 CUSTGRAFPATH=1 PG_PASS=... ./devel/deploy_all.sh`. If succeeded `make install`.
